...
;(function () {
    ...
    */
    window.animate = function (ele, target = {}, duration = 1000) {
        ...
        //计算当前位置
        function computedCurPosition(time) {
            //起始值
            let beginPosition = {};
            //总运动值
            let change = {};
            ...
            for (let attr in target) {
                if (target.hasOwnProperty(attr)) {
                    ...
                    beginPosition[attr] = utils.getCss(ele, attr);
                    change[attr] = target[attr] - beginPosition[attr];
                    curPosition[attr] = effect.linear(time, duration, change[attr], beginPosition[attr]);
                }
            }
            //返回根据时间计算出的当前位置值
            return curPosition;
        ...
        ele.animateTimer = setInterval(() => {
            ...
            //根据时间获取当前位置
            let curPosition = computedCurPosition(time);
            //设置元素的当前位置的各属性
